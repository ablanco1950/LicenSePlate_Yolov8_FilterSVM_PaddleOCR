...
modelSVM= pickle.load( open("./model.pickle", 'rb'))
...
def FindLicenseNumber (gray, x_offset, y_offset,  License, x_resize, y_resize, \
                       Resize_xfactor, Resize_yfactor, CodFilter):
#########################################################################
    ...
    if CodFilter==0:
        text =Codfilter0(gray,10)
        if text != "":
        
               TabLicensesFounded, ContLicensesFounded =ApendTabLicensesFounded (TabLicensesFounded, ContLicensesFounded, text)   
               if text==License:
                  print(text + "  Hit with image concat  z= 10" )
                  TotHits=TotHits+1
               else:
                   print(License + " detected with Filter image concat "+ text+ " z=10 ") 
        return TabLicensesFounded, ContLicensesFounded
    if CodFilter==1:
        text =Codfilter0(gray,11)
        if text != "":
        
               TabLicensesFounded, ContLicensesFounded =ApendTabLicensesFounded (TabLicensesFounded, ContLicensesFounded, text)   
               if text==License:
                  print(text + "  Hit with image concat  z= 11" )
                  TotHits=TotHits+1
               else:
                   print(License + " detected with Filter image concat "+ text+ " z=11 ") 
        return TabLicensesFounded, ContLicensesFounded
    if CodFilter==2:
        text =Codfilter2(gray)
        if text != "":
            TabLicensesFounded, ContLicensesFounded =ApendTabLicensesFounded (TabLicensesFounded, ContLicensesFounded, text)   
            if text==License:
...
with open( "LicenseResults.txt" ,"w") as  w:
    for i in range (len(imagesComplete)):
            ...
            grayCodFilter= cv2.cvtColor(gray, cv2.COLOR_BGR2GRAY)
            grayCodFilter = cv2.resize(grayCodFilter, (416,416), interpolation = cv2.INTER_AREA)     
            X_testFlat=[]
            X_testFlat.append(grayCodFilter.flatten())
            X_testFlat=np.array(X_testFlat)
            CodFilter=modelSVM.predict(X_testFlat)
            print(License + " Cod filter = "+ str(CodFilter))
            if CodFilter> 10:
                print("RAROOOOOOOOOOOOOOOO CODIGO MAYOR QUE 10")
                continue
...
print("")           
#print("Total Hits = " + str(TotHits ) + "Total Failures = " + str(TotFailures ) + " from " + str(len(imagesComplete)) + " images readed")
