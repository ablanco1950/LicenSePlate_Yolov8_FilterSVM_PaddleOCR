...
def discrete_radon_transform(image):
    ...
    R = np.zeros((w,steps), dtype='float64')
    #for s in range(steps):
    #print("steps" + str(steps))
    for s in range(steps):
        #if s > image.shape[0] -1: break
        ...
        #rotation = skimage.transform.rotate(image, -s*90/steps).astype('float64')
        rotation = skimage.transform.rotate(image, -s).astype('float64')
        #print ("PASA 1 VEZ")
        R[:,s] = sum(rotation)
    return R
...
def GetRotationImage(image):
    ...
    sinogram = radon(I)
    #sinogram = discrete_radon_transform(I)
...
"""
# https://github.com/dht7166/Brightness_Contrast
def Brightness_Contrast(image):
     from model import BRC
     model = BRC()
     model.model.load_weights('brc_1.h5')
     image= cv2.cvtColor(image,cv2.COLOR_GRAY2RGB)   
     #image = cv2.imread('WilburImage.jpg')
     image1 = cv2.resize(image,(256,256))
     image1 = np.divide(image1,255.0)
     image1 = np.expand_dims(image1,axis=0)
     pred = model.model.predict(image1)
     alpha = pred[0][0][0][0]
     beta = pred[0][0][0][1]
     print(alpha,beta)
...
#########################################################################
    ...
    """
    gray1 = Brightness_Contrast(gray)
    text, Accuraccy = GetPaddleOcr(gray1)    
    text = ''.join(char for char in text if char.isalnum())
    text=ProcessText(text)
    if ProcessText(text) != "":
            ...
            TabLicensesFounded, ContLicensesFounded =ApendTabLicensesFounded (TabLicensesFounded, ContLicensesFounded, text)   
            if text==License:
               print(text + "  Hit with Brightness_Contrast  ")
               TotHits=TotHits+1
               TabTotHitsFilter[0]=TabTotHitsFilter[0]+1
            else:
                print(License + " detected with Filter Brightness_Contrast "+ text) 
                TabTotFailuresFilter[0]=TabTotFailuresFilter[0]+1
    """
    ################################################################
...
for i in range (len(imagesComplete)):
        ...
        for x in range(len(TabImgSelect)):
            ...
            CodFilter = FindLicenseNumber (gray, x_off, y_off,  License, x_resize, y_resize, \
                                   Resize_xfactor, Resize_yfactor, BilateralOption)
            if  CodFilter == -1 : continue
            lineaw=[]
            cv2.imwrite(dirname_training+"\\" + License +".jpg",imagesComplete[i])
            with open( dirname_training+"\\" + License +".txt","w") as  w:
